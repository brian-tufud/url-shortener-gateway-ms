...
public class ShortURLController {
    ...
    @PostMapping()
    public ResponseEntity<CrudStatusResponse> shortenURL(HttpServletRequest request,
        @RequestBody LongURLRequest body) throws Exception {

        shortURLService.shortenURL(body);

        CrudStatusResponse response = CrudStatusResponse.builder().status(CrudStatus.CREATED).build();
